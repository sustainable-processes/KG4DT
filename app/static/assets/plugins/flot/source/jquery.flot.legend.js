...
(function($) {
    ...
    function insertLegend(plot, options, placeholder, legendEntries) {
        ...
        html[j++] = '<rect class="background" width="100%" height="100%" fill="rgba(255, 255, 255, 0.5)" stroke="black"/>';
        html[j++] = svgShapeDefs;
        ...
        for (i = 0; i < entries.length; ++i) {
            ...
            var info = plot.getSurface().getTextInfo('', shape.label.replace(/<[^>]+>/g, ''), {
            ...
            if (columnWidths[columnIndex]) {
                if (labelWidth + iconWidth > columnWidths[columnIndex]) {
        ...
        for (i = 0; i < entries.length; ++i) {
            ...
            shape.yPos = Math.floor(i / options.legend.noColumns) * 1.5 + 0.1 + 'em';
            // area
            ...
            labelHtml = '<text x="' + shape.xPos + '" y="' + shape.yPos + '" text-anchor="start"><tspan dx="2em" dy="1.1em">&nbsp;' + label + '</tspan></text>';
        ...
        if (p.charAt(1) === 'e') {
            pos += 'right:' + (m[0] + plotOffset.right + 20) + 'px;';
        } else if (p.charAt(1) === 'w') {
            pos += 'left:' + (m[0] + plotOffset.left) + 'px;';
        }
        
        var width = Math.max(columnWidths);
        // for (i = 0; i < columnWidths.length; ++i) {
        //     width += columnWidths[i];
        // }

        var legendEl,
            height = Math.ceil(entries.length / options.legend.noColumns) * 1.6 + 0.1;
