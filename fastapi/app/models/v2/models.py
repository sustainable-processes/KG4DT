...
class User(V2Base):
    ...
    created_at: Mapped = mapped_column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped = mapped_column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False)

    projects: Mapped[list[Project]] = relationship(back_populates="user", cascade="all, delete-orphan")  # type: ignore


class Project(V2Base):
...
class Reactor(V2Base):
    ...
    json_data: Mapped[dict] = mapped_column(JSONB, server_default="{}", nullable=False)
...
class ReactorBasicJunction(V2Base):
    ...
    association_at: Mapped = mapped_column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False)


class Category(V2Base):
