...
class GraphDBHandler:
    """Handler for interacting with a GraphDB database."""

    def __init__(self, config):
        """Initializes the GraphDBHandler.

        Args:
            config: Configuration containing GraphDB settings and ontology classes.
        """
        # Graphdb
        self.host = config.GRAPHDB_HOST
        self.port = config.GRAPHDB_PORT
        self.user = config.GRAPHDB_USER
        self.pw = config.GRAPHDB_PASSWORD
        ...
        self.conn = pygraphdb.connect(self.host, self.port, self.user, self.pw, self.db)
